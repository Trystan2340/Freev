...
def get_freev():
    global freev
    if freev is None:
        raise RuntimeError("Freev pas encore chargé")
    return freev
    ...
    def do_GET(self):
        ...
        if self.path == "/status":
            ...
            status = {
                ...
                "server": "freev-render",
                "uptime_ok": True,
                "version": get_freev().VERSION if freev else "loading",
                "trained": get_freev().brain.trained if freev else False,
                "pairs": len(get_freev().brain.training_data) if freev else 0,
                "personality": get_freev().personality if freev else "unknown",
            }
...
<h1>🤖 Freev AI Server v{get_freev().VERSION if freev else "loading"}</h1>
<p>✅ FreevBrain actif — {len(get_freev().brain.training_data) if freev else 0} paires</p>
...
<p>GET  /status   — vérifier l'état</p>
    ...
    def do_POST(self):
        ...
        try:
            ...
            # ── Réponse FreevBrain ──────────────────────────────────────────
            response = get_freev().generate_response(message)
            if not response:
...
if __name__ == "__main__":
    print("⏳ Chargement FreevBrain...", flush=True)
    try:
        from brain import Freev
        freev = Freev()
        print(f"✅ FreevBrain prêt — {len(get_freev().brain.training_data)} paires chargées", flush=True)
    except Exception as e:
    ...
    print("🚀 Server ready — listening", flush=True)
