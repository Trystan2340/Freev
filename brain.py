...
"""
╔══════════════════════════════════════════════════════════════════╗
║  FREEV v4.0 — IA Conversationnelle Locale                        ║
║  Modèle : FreevBrain v4 — Python pur, Zéro dépendance            ║
...
# ══════════════════════════════════════════════════════════════════════════════
# MODULE 5 — MLPBrain (Grok, bugs corrigés)
# Réseau de neurones feedforward + backprop + Adam from scratch
# ══════════════════════════════════════════════════════════════════════════════
class MLPBrain:
    """
    MLP feedforward from scratch.
    ...
    def __init__(self, input_dim: int, hidden_dim: int = 256, output_dim: int = 64):
        ...
        self.cache: dict = {}
        # Grok v5.0 : dropout + early stopping
    ...
    def _init_parameters(self):
        ...
        # Layer 3 : (output × hidden)  — W3[i][j] → z3[i] = sum_j W3[i][j]*a2[j]
    ...
    @staticmethod
    def _add_bias(z, b):
        return [zi + bi for zi, bi in zip(z, b)]
    ...
    def forward(self, x: list) -> list:
        z1 = self._add_bias(self._mat_vec_mul(self.W1, x), self.b1)
        a1 = self._relu(z1)
        z2 = self._add_bias(self._mat_vec_mul(self.W2, a1), self.b2)
        a2 = self._relu(z2)
        z3 = self._add_bias(self._mat_vec_mul(self.W3, a2), self.b3)
    ...
    def train_step(self, x: list, y_true: int) -> float:
        ...
        self.backward(y_true, self.lr)
        return loss

    def backward(self, y_true: int, lr: float = 0.001):
        self.t += 1
        c = self.cache
        probs, a2, a1, x = c['probs'], c['a2'], c['a1'], c['x']
        d_z3 = [probs[i] - (1.0 if i == y_true else 0.0) for i in range(self.output_dim)]
        # dW3[i][j] = d_z3[i] * a2[j]   (shape : output × hidden)
        ...
        d_z2 = [d_a2[i] * d for i, d in enumerate(self._relu_deriv(c['z2']))]
        # dW2[i][j] = d_z2[i] * a1[j]   (shape : hidden × hidden)
        ...
        d_z1 = [d_a1[i] * d for i, d in enumerate(self._relu_deriv(c['z1']))]
        # dW1[i][j] = d_z1[i] * x[j]    (shape : hidden × input)
        dW1 = [[d_z1[i] * x[j] for j in range(self.input_dim)] for i in range(self.hidden_dim)]
        db1 = d_z1[:]
        self._adam_update(self.W1, self.mW1, self.vW1, dW1, lr)
        self._adam_update_vec(self.b1, self.mb1, self.vb1, db1, lr)
        self._adam_update(self.W2, self.mW2, self.vW2, dW2, lr)
        self._adam_update_vec(self.b2, self.mb2, self.vb2, db2, lr)
        self._adam_update(self.W3, self.mW3, self.vW3, dW3, lr)
        self._adam_update_vec(self.b3, self.mb3, self.vb3, db3, lr)
    ...
    @staticmethod
    def _dropout(layer: list, rate: float = 0.2) -> list:
        """Dropout en training : met rate% des activations à 0. Désactivé en inférence."""
        if rate <= 0.0: return layer[:]
        if rate > 1.0: return [0.0] * len(layer)
        return [0.0 if random.random() < rate else v for v in layer]

    def confidence(self, x: list) -> float:
        """Retourne la confiance de la prédiction (max du softmax) — inférence uniquement."""
        probs = self.forward(x)
        return max(probs)

    def train_batch(self, examples: list, y_labels: list) -> float:
        """
        Entraîne sur un mini-batch : accumule les gradients, les moyenne,
        fait UNE SEULE mise à jour Adam. Retourne la loss moyenne.
        """
        if not examples or len(examples) != len(y_labels): return 0.0
        batch_size = len(examples)
        accum_dW1 = [[0.0]*self.input_dim  for _ in range(self.hidden_dim)]
        accum_db1 = [0.0]*self.hidden_dim
        accum_dW2 = [[0.0]*self.hidden_dim for _ in range(self.hidden_dim)]
        accum_db2 = [0.0]*self.hidden_dim
        accum_dW3 = [[0.0]*self.hidden_dim for _ in range(self.output_dim)]
        accum_db3 = [0.0]*self.output_dim
        total_loss = 0.0
        for x, y_true in zip(examples, y_labels):
            probs = self.forward(x)
            total_loss += -math.log(probs[y_true] + 1e-12)
            c = self.cache
            d_z3 = [c['probs'][i] - (1.0 if i == y_true else 0.0) for i in range(self.output_dim)]
            dW3_t = [[d_z3[i]*c['a2'][j] for j in range(self.hidden_dim)] for i in range(self.output_dim)]
            d_a2  = [sum(self.W3[i][j]*d_z3[i] for i in range(self.output_dim)) for j in range(self.hidden_dim)]
            d_z2  = [d_a2[i]*d for i,d in enumerate(self._relu_deriv(c['z2']))]
            dW2_t = [[d_z2[i]*c['a1'][j] for j in range(self.hidden_dim)] for i in range(self.hidden_dim)]
            d_a1  = [sum(self.W2[i][j]*d_z2[i] for i in range(self.hidden_dim)) for j in range(self.hidden_dim)]
            d_z1  = [d_a1[i]*d for i,d in enumerate(self._relu_deriv(c['z1']))]
            dW1_t = [[d_z1[i]*c['x'][j] for j in range(self.input_dim)] for i in range(self.hidden_dim)]
            for i in range(self.hidden_dim):
                for j in range(self.input_dim):  accum_dW1[i][j] += dW1_t[i][j]/batch_size
                accum_db1[i] += d_z1[i]/batch_size
            for i in range(self.hidden_dim):
                for j in range(self.hidden_dim): accum_dW2[i][j] += dW2_t[i][j]/batch_size
                accum_db2[i] += d_z2[i]/batch_size
            for i in range(self.output_dim):
                for j in range(self.hidden_dim): accum_dW3[i][j] += dW3_t[i][j]/batch_size
                accum_db3[i] += d_z3[i]/batch_size
        self.t += 1
        self._adam_update(self.W1, self.mW1, self.vW1, accum_dW1, self.lr)
        self._adam_update_vec(self.b1, self.mb1, self.vb1, accum_db1, self.lr)
        self._adam_update(self.W2, self.mW2, self.vW2, accum_dW2, self.lr)
        self._adam_update_vec(self.b2, self.mb2, self.vb2, accum_db2, self.lr)
        self._adam_update(self.W3, self.mW3, self.vW3, accum_dW3, self.lr)
        self._adam_update_vec(self.b3, self.mb3, self.vb3, accum_db3, self.lr)
...
class ContextManager:
    ...
    def extract_entities(self, text: str) -> dict:
        ...
        entities['names'] = [n for n in entities['names'] if n.lower() not in self.CITIES]
        return entities
...
class DataAugmentor:
    ...
    def augment(self, question: str, response: str, max_variants: int = 5) -> list:
        ...
        return variants if variants else [(question, response)]
...
class FreevTestSuite:
    ...
    def _call_brain(self, brain, question):
        ...
        if callable(brain):
            try:
                ...
                pass
        if result is None:
            for method in ("respond", "predict", "__call__"):
    ...
    def run_benchmark(self, brain, test_pairs: list) -> dict:
        ...
        for pair in test_pairs:
            ...
            match = self._norm_text(response) == self._norm_text(expected)
            if match:
                correct += 1
...
class FreevBrain:
    """
    FreevBrain v4 — Cerveau IA orchestrant tous les modules.
    Pipeline : BPE → Embeddings → BM25+Cosine+Jaccard+Levenshtein → KneserNey
    + KnowledgeGraph + ContextManager + DataAugmentor + MLPBrain (optionnel)
    """
    BRAIN_FILE = Path.home() / '.freev_brain_v4.json'
    KG_FILE    = Path.home() / '.freev_kg_v4.json'
    ...
    def _save(self):
        ...
        with open(self.BRAIN_FILE, 'w', encoding='utf-8') as f:
            ...
            json.dump({'trained': self.trained, 'data': data, 'mlp': mlp_data}, f,
                      ensure_ascii=False)
        ...
        if self.BRAIN_FILE.exists():
            try:
                ...
                for entry in saved.get('data', []):
                    t = self._tokenize(entry['question'])
    ...
    def respond(self, user_input: str):
        ...
        # 1. Résolution des références pronominales
        resolved = self.context.resolve_references(user_input)

        # 2. Inférence KnowledgeGraph
        tokens = self._tokenize(resolved)
        ...
        # 3. Enrichissement avec contexte de conversation
        ctx_tokens = self.context.get_context_tokens(n_turns=2)
        combined_tokens = tokens + [t for t in ctx_tokens if t not in tokens][:5]

        qv = self._build_vector(combined_tokens)
        q_ngrams = self._get_ngrams(combined_tokens, 3)
        N = len(self.training_data)

        # 4. Candidats via index inversé
        # Utiliser UNIQUEMENT les tokens de la question courante (pas le contexte)
        key_tokens = [t for t in tokens if len(t) > 1]

        # Détecter les tokens "inconnus" — absents de toute la base
        unknown_key_tokens = [t for t in key_tokens
                              if t not in self.inverted_index]
        if key_tokens and unknown_key_tokens:
            unknown_ratio = len(unknown_key_tokens) / len(key_tokens)
            # 1 token clé  → 1 inconnu = 100% → hors domaine
            # 2 tokens clés → 1 inconnu = 50% → hors domaine  
            # 3 tokens clés → 1 inconnu = 33% → hors domaine
            # Règle : si ≥ 33% des mots clés sont inconnus → hors domaine
            if unknown_ratio >= 0.80:
                return None

        search_tokens = key_tokens if key_tokens else tokens
        candidates = set()
        for t in search_tokens:
            candidates.update(self.inverted_index.get(t, []))

        # Fallback : tokens courts si rien avec les longs
        if not candidates:
            for t in tokens:
                candidates.update(self.inverted_index.get(t, []))

        # exact-match sur texte brut — questions très courtes
        if not candidates:
            tl_clean = re.sub(r"[^\w\s]", '', user_input.lower()).strip()
            for i, e in enumerate(self.training_data):
                q_clean = re.sub(r"[^\w\s]", '', e['question'].lower()).strip()
                if tl_clean == q_clean or tl_clean in q_clean:
                    candidates.add(i)
        if not candidates:
            return None

        # 5. Scoring multi-métriques parmi les candidats pertinents
        best_score = -1.0
        best_response = None
        best_entry_tokens = []

        # Longueur de la question originale (en mots bruts, avant stop words)
        raw_query_len = len(user_input.split())

        # FIX perf : vecteur embedding de la requête calculé UNE FOIS avant la boucle
        q_emb_mean = None
        if self.embedder.dim > 0 and combined_tokens:
            q_vecs = [self.embedder.get_vector(t) for t in combined_tokens if t in self.embedder.vocab]
            if q_vecs:
                dim = self.embedder.dim
                q_emb_mean = {str(i): sum(v[str(i)] for v in q_vecs) / len(q_vecs) for i in range(dim)}

        for idx in candidates:
            e = self.training_data[idx]
            bm  = self.scorer.bm25(combined_tokens, e['tokens'], e['length'],
                                   self.avg_doc_len, self.df, N)
            cos = self.scorer.cosine(qv, e['vector'])
            jac = self.scorer.jaccard_ngrams(q_ngrams, e['ngrams'])
            lev = self.scorer.levenshtein_ratio(resolved.lower(), e['question'].lower())

            # Score sémantique via embeddings (calculé efficacement)
            emb_score = 0.0
            if q_emb_mean and e['tokens']:
                e_vecs = [self.embedder.get_vector(t) for t in e['tokens'] if t in self.embedder.vocab]
                if e_vecs:
                    dim = self.embedder.dim
                    e_mean = {str(i): sum(v[str(i)] for v in e_vecs) / len(e_vecs) for i in range(dim)}
                    emb_score = self.scorer.cosine(q_emb_mean, e_mean)

            # Bonus : mots clés substantiels présents dans la question candidate
            key_hits = sum(1 for t in key_tokens if t in e['tokens'])
            key_bonus = (key_hits / max(len(key_tokens), 1)) * 0.5

            # Bonus longueur : préférer les questions de longueur similaire
            # Évite que "c'est quoi le bitcoin" → "c'est quoi le minage de bitcoin"
            cand_len = len(e['question'].split())
            len_ratio = min(raw_query_len, cand_len) / max(raw_query_len, cand_len)
            len_bonus = len_ratio * 0.4  # jusqu'à +0.4 pour longueur identique

            # Bonus Levenshtein fort quand les tokens sont peu nombreux
            # (query courte = le ratio de similarité des chars compte beaucoup)
            lev_bonus = lev * 0.5 if len(tokens) <= 2 else lev * 0.2

            # Gemini FIX — score final intègre désormais les embeddings (0.1 de poids)
            score = self.scorer.combined_score(bm, cos, jac, lev) + key_bonus + len_bonus + lev_bonus + emb_score * 0.1

            if score > best_score:
                best_score = score
                best_response = e['response']
                best_entry_tokens = e['tokens']

        # Validation finale : proportion de tokens clés en commun
        if key_tokens:
            real_overlap = sum(1 for t in key_tokens if t in best_entry_tokens)
            overlap_ratio = real_overlap / len(key_tokens)
            # Règle stricte par nombre de tokens :
            # 1 token clé  → 100% requis (le mot DOIT être dans l'entrée)
            # 2 tokens clés → 100% requis (les 2 mots doivent être présents)
            # 3+ tokens clés → 60% requis
            if len(key_tokens) <= 2:
                min_ratio = 1.0
            else:
                min_ratio = 0.6
        ...
        # Gemini FIX — MLP valide / booste le meilleur candidat
        if self.mlp and best_response and self.vocab_to_idx:
            try:
                x = self._to_bow_vector(combined_tokens)
                probs = self.mlp.forward(x)
                # Indice de la paire best_response dans training_data
                best_idx = next(
                    (i for i, e in enumerate(self.training_data) if e['response'] == best_response),
                    None
                )
                if best_idx is not None and best_idx < len(probs):
                    # Si MLP est confiant sur ce candidat → boost de score
                    mlp_conf = probs[best_idx]
                    best_score += mlp_conf * 0.3  # bonus MLP jusqu'à +0.3
            except Exception:
                pass  # MLP en erreur → ignorer silencieusement
        ...
        if best_score > dynamic_threshold and best_response:
            self.context.add_turn(user_input, best_response)
            return best_response

        return None  # Laisse Freev utiliser son fallback de personnalité
    ...
    def train_mlp(self, epochs: int = 10):
        ...
        self.mlp = MLPBrain(input_dim=input_dim, hidden_dim=128, output_dim=output_dim)
    ...
    def status(self) -> str:
        if self.trained:
            ...
            return (f"{C.BR_GREEN}✓ FreevBrain v4{C.RESET} "
    ...
    # ── Auto-apprentissage (appelé par server.py après chaque réponse) ────────
    def learn(self, question: str, reponse: str):
        """
        Ajoute une paire question/réponse en mémoire vive.
        Construit l'entrée complète (tokens, vector, ngrams) pour rester
        compatible avec respond() et les scorers existants.
        """
        question = question.strip()
        reponse  = reponse.strip()
        if not question or not reponse:
            return
        # Construire l'entrée au même format que training_data
        ...
        self.training_data.append(entry)
        # Mise à jour incrémentale des index (sans tout reconstruire)
        for t in set(tokens):
            self.df[t] = self.df.get(t, 0) + 1
            self.vocabulary.add(t)
        total_len = sum(e['length'] for e in self.training_data)
        self.avg_doc_len = total_len / len(self.training_data)
        self.trained = True
    ...
    def save(self):
        ...
        try:
            if self.training_data:
                ...
                with open('freev_data.txt', 'a', encoding='utf-8') as f:
                    f.write(f"\n{last['question']}=>{last['response']}")
        except Exception as e:
...
class Freev:
    """
    Freev v4.0 — Interface conversationnelle de terminal.
    ...
    def _save_memory(self):
        try:
            data = {'notes': self.notes, 'reminders': self.reminders, 'memory': self.memory}
...
{C.BG_BLUE}{C.BR_WHITE}{'═'*60}{C.RESET}
{C.BR_CYAN}  FreevBrain v4 : BPE · Embeddings · BM25 · KneserNey · MLP{C.RESET}
...
{greeting}""")
    ...
    def handle_brain(self, text: str):
        ...
        if 'entraîne toi' in tl or 'entraine toi' in tl or 'train brain' in tl:
            ...
            if ok:
                ...
                msg = f"✅ FreevBrain v4 entraîné ! {n} paires (avec augmentation).\n"
        ...
        # Benchmark sur freev_data.txt (utilise les paires comme test)
        if any(x in tl for x in ['benchmark', 'test brain', 'teste brain', 'évalue brain', 'evalue brain']):
            ...
            pairs = self.brain.tester.load_test_file(str(self.brain._find_data_file() or ''))
            if not pairs:
                return "❌ Aucune paire de test trouvée dans freev_data.txt."
            # Limiter à 50 paires pour vitesse
            sample = pairs[:50]
            print(f"  🧪 Benchmark sur {len(sample)} paires...")
            metrics = self.brain.tester.run_benchmark(self.brain, sample)
            return (f"📊 Benchmark FreevBrain ({len(sample)} paires) :\n"
    ...
    def handle_text_analysis(self, text: str):
        ...
        if 'palindrome' in tl:
            ...
            if m2:
                ...
                is_p = word == word[::-1]
                return f"{'\u2705 Oui' if is_p else '\u274c Non'}, '{m2.group(1)}' est {'un' if is_p else 'pas un'} palindrome."
    ...
    def handle_wikipedia(self, text: str):
        ...
        query = None
        for prefix in ['wikipedia ', 'wiki ', 'cherche ', 'recherche ',
                        'que sais-tu sur ', 'parle moi de ', 'parle-moi de ',
                        "dis moi qui est ", "c'est qui ", 'kesako ', 'explique moi ']:
        ...
        return self._wiki_format(title, extract, url, learned)
    ...
    def handle_voice(self, text: str):
        ...
        if any(x in tl for x in ['voix on', 'active la voix', 'commence à parler', 'parle moi']):
            ...
            return "🔊 Mode vocal activé ! Je vais maintenant parler mes réponses."
        ...
        if any(x in tl for x in ['écoute', 'ecoute', 'mode vocal', 'parle moi freev', 'active micro']):
            ...
            return "🎤 Mode écoute activé ! Parle, je t'écoute."
...
{C.BR_YELLOW}🧠 FREEVBRAIN{C.RESET}
  ...
  benchmark        → évaluer la précision sur freev_data.txt
...
{C.BR_CYAN}{'─'*55}{C.RESET}"""
    ...
    def _try_wikipedia_fallback(self, text: str):
        ...
        query = None
        patterns = [
            (r"c'est quoi (?:le|la|les|l'|un|une)?\s*(.+)", 1),
            (r"c'est quoi\s+(.+)", 1),
            (r"qu'est.ce que (?:le|la|les|l'|un|une)?\s*(.+)", 1),
            (r"qu'est.ce qu'(?:un|une)?\s*(.+)", 1),
            (r"qui (?:est|etait|était) (?:le |la |l')?(.+)", 1),
            (r"qui est-(?:ce que |ce qu')?(.+)", 1),
            (r"comment fonctionne\s+(?:un|une|le|la)?\s*(.+)", 1),
            (r"(.+) c'est quoi", 1),
            (r"parle.?moi de (.+)", 1),
            (r"(?:dis.?moi|raconte.?moi|explique.?moi) (?:ce qu'est |ce que )?(.+)", 1),
        ]
        for pattern, group in patterns:
            m = re.search(pattern, tl)
            if m:
                query = m.group(group).strip().rstrip('?').strip()
                break
    ...
    def generate_response(self, user_input: str) -> str:
        ...
        intent_handlers = {
            'recherche':    [self.handle_wikipedia, self.handle_weather],
            'commande':     [self.handle_conversions, self.handle_math,
                             self.handle_system, self.handle_text_analysis,
                             self.handle_games],
            'question':     [self.handle_wikipedia, self.handle_math,
        ...
        remaining_handlers = [
            self.handle_wikipedia, self.handle_weather, self.handle_conversions,
        ...
        brain_response = self.brain.respond(text)
        if brain_response:
            # Diversifier : éviter de répéter la même réponse
            diversified = self.brain.diversifier.filter(
                [brain_response] + [brain_response]  # seule candidate ici
            )
